...
do_record_single_image() {
    ...
    if [ ! -z "${USE_SKOPEO}" ]; then
        oc_cmd="skopeo copy ${SK_RETRY_STRING} --all --authfile \"${AUTH_DATA_PATH}/auth.json\" --dest-tls-verify=false --src-tls-verify=false \"docker://${IMAGE}\" \"docker://${TARGET_REGISTRY}/${image_identifier}\""
    else
        oc_cmd="oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" \"${IMAGE}\" \"${TARGET_REGISTRY}/${image_identifier}\" --filter-by-os '.*' --insecure"
    fi
    commands=("$oc_cmd")
    print_dry_run_commands "${commands[@]}"
...
do_image_mirror_single_image() {
    ...
    oc_cmd="oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" \"${IMAGE}\" \"${TARGET_REGISTRY}/${image_identifier}\" --filter-by-os '.*' --insecure ${OC_DEBUG_LOGGING}"
...
do_skopeo_copy_single_image() {
    ...
    oc_cmd="skopeo copy ${SK_RETRY_STRING} --all --authfile \"${AUTH_DATA_PATH}/auth.json\" --dest-tls-verify=false --src-tls-verify=false \"docker://${IMAGE}\" \"docker://${TARGET_REGISTRY}/${image_identifier}\" ${SKOPEO_DEBUG_LOGGING}"
    log_debug "Running: ${oc_cmd}"
    eval ${oc_cmd}
...
do_record_case_images() {
    ...
    if [ -z "${DRY_RUN_OUTPUT}" ]; then
        ...
        for map_file in ${map_files}; do
            cat ${map_file}
            commands+=("oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" -f \"${map_file}\" --filter-by-os '.*' --insecure")
            if [[ "$?" -ne 0 ]]; then
        ...
        for map_file in ${map_files}; do
            ...
            commands+=("oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" -f \"${dir_name}/${mapping_filename}.txt\" --filter-by-os '.*' --insecure")
            if [[ "$?" -ne 0 ]]; then
                # On error we need to clean up our mirrored csv file
                cleanup_mirrored_csv_files_from_current_run
                exit 11
    ...
    if [ ! -z "${USE_SKOPEO}" ]; then
        ...
        while read in; do
            commands+=("skopeo copy ${SK_RETRY_STRING} --all --authfile \"${AUTH_DATA_PATH}/auth.json\" --dest-tls-verify=false --src-tls-verify=false docker://${in}")
...
do_image_mirror_case_images() {
    ...
    for map_file in ${map_files}; do
        log_info "Mirroring ${map_file}"
        oc_cmd="oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" -f \"${map_file}\" --filter-by-os '.*' --insecure ${OC_DEBUG_LOGGING}"
        log_debug "Running: ${oc_cmd}"
        eval ${oc_cmd}
...
do_skopeo_copy_case_images() {
    ...
    find_images_to_mirror
    while read in; do
        oc_cmd="skopeo copy ${SK_RETRY_STRING} --all --authfile \"${AUTH_DATA_PATH}/auth.json\" --dest-tls-verify=false --src-tls-verify=false docker://${in} ${SKOPEO_DEBUG_LOGGING}" ;
                ...
                if [[ "$(is_valid_olm_catalog_tag ${tag})" == "true" ]]; then
                    ...
                    if [[ ! -z "${all_tags}" ]]; then
                        ...
                        if [[ ${last_tag} =~ ^${tag_significane} ]]; then
                            ...
                            oc_cmd="oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" \"${image}@${sha}\" \"${image}:${latest_tag}\" --filter-by-os '.*' --insecure  ${OC_DEBUG_LOGGING}"
                            log_debug "Running: ${oc_cmd}"
                            eval ${oc_cmd}
